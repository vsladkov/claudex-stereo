...
test("reapLeakedTestBrokers kills tmp-cwd brokers and spares real workspaces", { skip: !IS_LINUX }, async () => {
  ...
  try {
    ...
    assert.equal(processAlive(realBroker.pid), true, "sweep must never touch a real-workspace broker");
  } finally {
