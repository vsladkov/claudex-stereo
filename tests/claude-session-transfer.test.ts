...
const { resolveClaudeSessionPath } =
  await import('../plugins/stereo/src/workspace/claude-session-transfer.ts');
...
test('a symlink inside the projects dir cannot escape it', { skip: IS_WINDOWS }, () => {
    ...
    assert.throws(
      () =>
        resolveClaudeSessionPath(cwd, {
          source: path.join(movedAside, 'workspace', 'session-b.jsonl'),
        }),
