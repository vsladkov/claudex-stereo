...
import {
  ...
  readOutputSchema,
} from '../plugins/stereo/src/runtime/structured-output.ts';
...
test('readOutputSchema reads the shipped review output schema', () => {
  const schema = readOutputSchema(path.join(SCHEMAS_DIR, 'review-output.schema.json')) as Record<
    string,
    any
  >;
...
test('readOutputSchema reads the shipped plan-review output schema', () => {
  const schema = readOutputSchema(
    path.join(SCHEMAS_DIR, 'plan-review-output.schema.json'),
