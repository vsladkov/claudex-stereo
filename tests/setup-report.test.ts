...
import {
  buildAppServerAuthStatus
} from "../plugins/stereo/src/runtime/auth.ts";
import type {
  CodexAuthStatus,
  ConfiguredProvider
} from "../plugins/stereo/src/runtime/auth.ts";
...
test("configured providers report ready aliases without provider next steps when all keys are set", async () => {
  ...
  assert.equal(report.nextSteps.some((step) => /third-party aliases|configured provider/.test(step)), false);
});

test("configured providers name an exact missing environment key", async () => {
  const env = Object.fromEntries(
    PROVIDERS.filter((provider) => provider.id !== "moonshot").map((provider) => [provider.envKey, "test-key"])
  );
  const report = await buildSetupReport(
    makeTempDir(),
    [],
  ...
  assert.equal(
    report.nextSteps.some((step) => step.includes("Set MOONSHOT_API_KEY for configured provider moonshot")),
    true
  );
  assert.equal(
...
test("an unconfigured provider is represented once in the optional aliases summary", async () => {
  ...
  const env = Object.fromEntries(configuredProviders.map((provider) => [provider.envKey, "test-key"]));
  const report = await buildSetupReport(
    makeTempDir(),
    [],
...
test("logged-out auth parsing preserves configured providers for setup", async () => {
  ...
  assert.deepEqual(auth.configuredProviders, [
    { id: "moonshot", envKey: "MOONSHOT_API_KEY" }
  ]);
  assert.equal(
    report.providers.aliases.find((entry) => entry.alias === "kimi")?.configured,
    true
  );
});
