...
- Leave model unset by default. Only add `--model` when the user explicitly asks for a specific model, then pass that value through verbatim — a model id or one of the plugin aliases (`spark`, `sol`, `terra`, `luna`, `kimi`, `qwen`, `deepseek`, `glm`); the runtime resolves aliases itself, and `/stereo:setup` shows each provider alias's readiness. The value may carry an optional `codex:` prefix, which is also forwarded verbatim because the runtime strips it.
