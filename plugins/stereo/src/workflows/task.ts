...
export async function waitForSingleJobSnapshot(cwd: string, reference: string, options: WaitForSingleJobOptions = {}): Promise<AwaitedJobSnapshot> {
...
export async function resolveLatestTrackedTaskThread(cwd: string, options: ResolveLatestTaskThreadOptions = {}): Promise<{ id: string } | null> {
...
export function buildTaskRunMetadata({ prompt, resumeLast = false }: { prompt?: string | null; resumeLast?: boolean }): TaskRunMetadata {
...
export async function executeTaskRun(request: TaskRunRequest): Promise<CompanionExecution> {
  ...
  const failureMessage = (result.error as { message?: string } | null | undefined)?.message ?? result.stderr ?? "";
