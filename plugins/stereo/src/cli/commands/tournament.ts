...
export function handleTournamentState(argv: string[]): void {
  ...
  if (!action) {
    const state = readTournamentStateFile(workspaceRoot);
    if (state.parseError) {
      const statePath = resolveTournamentStateFile(workspaceRoot);
      const payload = {
        available: false,
        unreadable: true,
        path: statePath,
      ...
      const rendered = `Tournament state exists but is unreadable at ${statePath}: ${state.parseError}\nA new /stereo:tournament run replaces it; /stereo:tournament --resume cannot use it.\n`;
  ...
  if (action === 'record') {
    ...
    const input = readStatePayload(
      cwd,
      options['state-file'],
      'Store bounded contestant summaries instead of verbatim reports.',
    );
    ...
    // A patch must not weaken what --record validated.
