...
import { interruptAppServerTurn, releaseThreadReservationForCancelledJob } from "../../runtime/index.ts";
...
export async function handleCancel(argv: string[]): Promise<void> {
  ...
  const requestThreadId = (existing.request as { threadId?: string | null } | null | undefined)?.threadId ?? null;
