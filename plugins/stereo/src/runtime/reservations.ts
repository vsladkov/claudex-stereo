...
export interface StrandedReservationEntry {
  kind: "stranded-reservation" | "stranded-cleanup" | "orphaned-claim" | "unreadable" | "scan-error";
  lockPath?: string;
  claimPath?: string;
  threadId?: string;
  jobId?: string | null;
...
function readValidatedReservationRecord(recordPath: string, kind: "lock"): ValidatedReservationRecord<ReservationLockRecord>;
function readValidatedReservationRecord(recordPath: string, kind: "claim"): ValidatedReservationRecord<ReservationClaimRecord>;
function readValidatedReservationRecord(
  recordPath: string,
  kind: "lock" | "claim"
): ValidatedReservationRecord<ReservationLockRecord> | ValidatedReservationRecord<ReservationClaimRecord> {
  try {
...
export function listStrandedThreadReservations(): StrandedReservationEntry[] {
  ...
  const pairs = new Map<string, { lockPath: string; claimPath: string; lock: boolean; claim: boolean }>();
...
export function describeStrandedReservation(entry: StrandedReservationEntry | null | undefined): string {
  switch (entry?.kind) {
...
export function acquireThreadReservation(threadId: string | null | undefined, meta: ThreadReservationMeta = {}): ThreadReservation {
...
function candidateReservationPaths({ threadId, requestThreadId, jobId, pid }: CancelledJobIdentity): Array<{
  lockPath: string;
