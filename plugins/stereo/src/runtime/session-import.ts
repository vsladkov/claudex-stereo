...
function importedThreadIdForSource(sourcePath: string): string | null {
  ...
  const records: Array<ImportLedgerRecord | null | undefined> = Array.isArray(ledger?.records) ? ledger.records : [];
  const match = records
    .filter(
      (record) =>
...
function externalAgentSessionMigration(sourcePath: string, cwd: string): ExternalAgentConfigImportParams {
...
async function requestExternalAgentSessionImport(client: AppServerClient, params: ExternalAgentConfigImportParams): Promise<void> {
