...
function parseCompleteNullTerminatedPaths(output: unknown): string[] {
  ...
  while (start < buffer.length) {
    ...
    if (terminator > start) {
      files.push(buffer.subarray(start, terminator).toString('utf8'));
    }
    start = terminator + 1;
  }
...
export function listRepositoryFiles(
  cwd: string,
  {
    maxBufferBytes = DEFAULT_REPOSITORY_FILES_MAX_BUFFER_BYTES,
    runImpl = spawnSync,
  }: ListRepositoryFilesOptions = {},
  ...
  try {
    ...
    const result = runImpl(
      ...
      {
        encoding: 'buffer',
        maxBuffer: normalizeRepositoryFilesMaxBufferBytes(maxBufferBytes),
        shell: false,
        windowsHide: true,
      },
    );
...
function gitPathList(cwd: string, args: readonly string[]): string[] {
  ...
  // still replace invalid UTF-8 bytes either way.
  ...
  if (result.error || result.status !== 0) {
    ...
    throw new Error(
      stderr || `git ${args[0]} failed${result.status != null ? ` (exit ${result.status})` : ''}.`,
    );
  }
...
function formatUntrackedFiles(cwd: string, files: string[], budgetBytes: number): string {
  const sections: string[] = [];
  let usedBytes = 0;
  for (let index = 0; index < files.length; index += 1) {
    const section = formatUntrackedFile(cwd, files[index] as string);
    const sectionBytes = Buffer.byteLength(section, 'utf8');
    if (usedBytes + sectionBytes > budgetBytes) {
      sections.push(
        `(+${files.length - index} more untracked file(s) omitted: aggregate limit of ${budgetBytes} bytes reached)`,
      );
      break;
    }
...
export function collectReviewContext(
  ...
  if (target.mode === 'working-tree') {
    ...
    details = collectWorkingTreeContext(repoRoot, state, {
      includeDiff,
      maxUntrackedTotalBytes: maxInlineDiffBytes,
    });
  } else {
