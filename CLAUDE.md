...
- `scripts/bump-version.ts`, `scripts/provider-probe.ts`, and `tests/` —
  dev-only, never shipped. The provider probe backs `npm run provider-probe`.
...
- `npm test` — node:test over 33 `tests/*.test.ts` files, one process per
  ...
  `env-bootstrap.cjs` with `--require`; it clears leaked `CLAUDE_PLUGIN_DATA`,
  `CLAUDE_ENV_FILE`, and `CLAUDE_PROJECT_DIR` values and redirects `CODEX_HOME`
  to temporary storage before tests so they cannot touch real user state.
