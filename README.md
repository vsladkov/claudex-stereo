...
such as `sol`, `spark`, `terra`, and `luna`, raw model ids, and qualified `model@provider` ids.
`--effort` applies only to Codex selections.
...
file-edit agent. `--effort` applies only to Codex-side selections.
